...
def Fn_Load_Miss_Check(row_data,con):
                        getresult=' select count(*)  from   '+str(row_data[2])+'  where   load_dt<>cast(getdate() as Date)'
                        curs_inner=con.cursor()
                        curs_inner.execute(getresult)
                        print(getresult)
                        qry=''
...
if __name__ == "__main__":
        ...
        user = "prakash"
        host = "db.internal.mycompany.com"
        port = "1212"
        sid = "db"

        pswd = "password"

        con = cx_Oracle.connect (user, pswd, 'db.internal.mycompany.com:12122/db')
        pswd = "password"
